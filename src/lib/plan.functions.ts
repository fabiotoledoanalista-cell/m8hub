  ...
  .inputValidator((d: {
    ...
    const clean = list
      .map((c) => ({
        ...
        observacao: (c.observacao ?? "")?.toString().trim() || null,
        tags: (c.tags ?? "")?.toString().trim() || null,
        valor: c.valor != null && c.valor !== "" ? Number(String(c.valor).replace(",", ".")) || null : null,
        origem: (c.origem ?? "")?.toString().trim() || null,
      }))
      .filter((c) => c.numero.length >= 8);
    return { contatos: clean, stage_id: d.stage_id ?? null };
  })
  .handler(async ({ context, data }) => {
    const companyId = await resolveCompanyId(context.supabase, context.userId);
    const { supabaseAdmin } = await import("@/integrations/supabase/client.server");
    ...
    // Resolve stage: usa stage_id informado ou primeiro stage da empresa
    let stageId: string | null = data.stage_id ?? null;
    let stageNome: string | null = null;
    if (stageId) {
      const { data: st } = await supabaseAdmin
        .from("crm_stage").select("id,nome").eq("id", stageId).eq("company_id", companyId).maybeSingle();
      stageNome = st?.nome ?? null;
      if (!stageNome) stageId = null;
    }
    if (!stageId) {
      const { data: firstStage } = await supabaseAdmin
        .from("crm_stage").select("id,nome").eq("company_id", companyId).order("ordem", { ascending: true }).limit(1).maybeSingle();
      stageId = firstStage?.id ?? null;
      stageNome = firstStage?.nome ?? null;
    }
    ...
    const payload = data.contatos.map((c) => {
      ...
      return {
        ...
        valor: c.valor,
        origem: c.origem || "csv",
        status: stageNome ?? "Conversas",
        stage_id: stageId,
        ultima_em: new Date().toISOString(),
      };
    });
    if (payload.length === 0) return { ok: true, inseridos: 0, atualizados: 0 };

    const { error } = await supabaseAdmin
      .from("crm_cards")
      .upsert(payload, { onConflict: "company_id,numero", ignoreDuplicates: false });
    if (error) throw new Error(error.message);
    return { ok: true, inseridos: novos.length, atualizados: data.contatos.length - novos.length };
  });
