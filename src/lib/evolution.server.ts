...
const SUPPORT_SUFFIX = " Se persistir, fale com o suporte: https://wa.me/5562994129808";
...
export async function evoDownloadMedia(instanceName: string, messageId: string): Promise<{ base64: string; mimetype: string } | null> {
  try {
    const data: any = await evo(`/chat/getBase64FromMediaMessage/${encodeURIComponent(instanceName)}`, {
      method: "POST",
      json: { message: { key: { id: messageId } }, convertToMp4: false },
    });
    if (data?.base64 && data?.mimetype) return { base64: data.base64, mimetype: data.mimetype };
    return null;
  } catch {
    return null;
  }
