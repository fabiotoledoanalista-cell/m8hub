...
function Landing() {
  async function cta(path: "/entrar" | "/demo/dashboard" | "#planos", plano?: string) {
    ...
    if (path === "/entrar") {
      ...
      window.location.href = plano
        ? `/entrar?modo=signup&plano=${plano}`
        : "/entrar?modo=login";
      return;
    }
  ...
  return (
    <div
      className="lp-root is-dark min-h-screen w-full antialiased overflow-x-hidden"
    ...
    >
      ...
      <style>{`
        ...
        .lp-root { font-family: 'Plus Jakarta Sans', system-ui, sans-serif; }
...
function useTheme() {
  ...
  useEffect(() => {
    // Design system ToledoTech é sempre dark — força dark na landing
    setIsDark(true);
  }, []);
  function toggle() {
    const next = !isDark;
