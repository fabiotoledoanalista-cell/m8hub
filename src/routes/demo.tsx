    ...
    <div className="min-h-screen flex flex-col bg-background text-foreground">
      <header className="border-b border-[color:var(--hairline)] bg-[color:var(--panel)]/85 backdrop-blur-xl px-4 py-2.5 text-[12.5px] md:text-[13.5px] flex items-center justify-between gap-3 sticky top-0 z-20">
        ...
        <div className="flex items-center gap-2 shrink-0">
          ...
          <Link
            to="/entrar"
            className="text-[12.5px] md:text-sm font-semibold px-4 py-1.5 rounded-full whitespace-nowrap transition-all hover:opacity-90"
            style={{ background: "var(--neonGreen)", color: "#0A1F14", boxShadow: "0 0 16px -4px rgba(52,211,153,.5)" }}
          >
            Criar conta
          </Link>
        </div>
      ...
      <div className="md:hidden flex items-center gap-2.5 px-4 py-3 border-b border-[color:var(--hairline)]">
        <div className="font-display font-bold text-[14.5px]">{brand.name}</div>
        <div className="text-[10.5px] uppercase tracking-[0.12em] text-muted-foreground">Demo · Padaria</div>
      </div>
      ...
      <div className="flex flex-1 flex-col md:flex-row">
        <aside className="hidden md:flex w-[260px] min-h-screen border-r border-[color:var(--hairline)] bg-[color:var(--sidebar-bg)] flex-col">
          <div className="px-5 py-5 flex items-center gap-3 border-b border-[color:var(--hairline)]">
            <div className="font-display font-extrabold text-[16px]">{brand.name}</div>
            <div className="text-[11px] uppercase tracking-[0.12em] text-muted-foreground">DEMO · Padaria</div>
          </div>
